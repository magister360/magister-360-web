...
export async function get(
  ...
  const { idUsuario, idMateria, idGrado, idGrupo, noPeriodo, estatus } =
    req.query;
  ...
  try {
    ...
    const formattedExamenes = examenes.map(
      ({ id, noPeriodo, calificacion, alumno }) => ({
        id,
        noPeriodo,
        calificacion,
    ...
    return res.status(200).json(formattedExamenes);
  } catch (error) {
    console.error(error);
