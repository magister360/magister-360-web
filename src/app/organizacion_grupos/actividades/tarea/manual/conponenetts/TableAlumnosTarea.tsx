...
import {
  StudentTareaType,
  TypeTareaCalificacion,
} from "@/app/types/tarea/TypeTarea";
import { EstatusTareaType } from "@/app/estatus/EstatusType";
import { createTarea, updateTarea } from "../../controller/TareaController";
import { updateEstatusTarea } from "../controller/AlumnoController";

type Props = {
  readonly students: StudentTareaType[] | null;
  readonly date: string | null;
  readonly tareas: TypeTareaCalificacion[] | null;
  readonly setIsFetchTarea: Dispatch<SetStateAction<boolean>>;
};
...
export default function TableAlumnosTarea({
  ...
  const handleConfirm = async () => {
    setIsModalConfirmOpen(false);
    const save = await updateEstatusTarea(id, EstatusTareaType.ELIMININAR);
    if (save.isSave) {
      setSuccessMessage(save.message);
      setIsSuccessModalOpen(true);
      setIsFetchTarea(true);
    } else {
  ...
  const handleConfirmOpen = async (noLista: number) => {
    ...
    const selectId = getId(tareas, noLista);
    setId(selectId);
  };
  ...
  const handleEditClick = (noLista: number | undefined) => {
    ...
    const selectId = getId(tareas, noLista);
    setId(selectId);
    const calificacion = getCalificacion(tareas, noLista);
  ...
  const getIdAlumno = (
    students: StudentTareaType[] | null,
  ...
  const getId = (
    ...
    const tarea = tareas?.find((p) => p.noLista === noLista);
    return tarea?.id ?? undefined;
  };
